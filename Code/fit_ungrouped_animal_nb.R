...
h = "model{
        for(i in 1:N){
            y[i] ~ dnegbin(1/(1 + mu[i]*phi), 1/phi)
        ...
        for(p in 1:N.Image){
            a1.image[p] ~ dnorm(a1.animal[Animal_ID[p]], a1.tau.image) T(, 0)
            a2.image[p] ~ dnorm(a2.animal[Animal_ID[p]], a2.tau.image) T(, 0)
            g.image[p] ~ dnorm(g.animal[Animal_ID[p]], g.tau.image) T(0, 98)
            t.image[p] ~ dnorm(t.animal[Animal_ID[p]], t.tau.image) T(0,)
            k1.image[p] ~ dnorm(k1.animal[Animal_ID[p]], k1.tau.image) T(1,)
            k2.image[p] ~ dnorm(k2.animal[Animal_ID[p]], k2.tau.image) T(1,)
        }
        for(l in 1:N.Animal){
            ...
            k1.animal[l] ~ dnorm(k1.pop, k1.tau.animal) T(1,)
            k2.animal[l] ~ dnorm(k2.pop, k2.tau.animal) T(1,)
        }
    
        #log gamma prior on phi
...
inits_func = function(chain, N.Animal, N.Image, N.Cell){
    gen_list = function(chain = chain){
        out = list(
            ...
            k1 = truncnorm::rtruncnorm(N.cell, a = 1, mean = 2, sd = 0.1),
            k2 = truncnorm::rtruncnorm(N.cell, a = 1, mean = 2, sd = 0.1)
        )
        out[['a1.pop']] = mean(out$a1)
        out[['a2.pop']] = mean(out$a2)
        out[['g.pop']] = mean(out$g)
        out[['t.pop']] = mean(out$t)
        out[['k1.pop']] = mean(out$k1)
        out[['k2.pop']] = mean(out$k2)
        ...
        out[['k1.sd.cell']] = LaplacesDemon::rhalft(1, scale = 0.1, nu = 4)
        out[['k2.sd.cell']] = LaplacesDemon::rhalft(1, scale = 0.1, nu = 4)
        ...
        out[['k1.image']] = rep(mean(out$k1), N.Image)
        out[['k2.image']] = rep(mean(out$k2), N.Image)
        ...
        out[['k1.sd.image']] = LaplacesDemon::rhalft(1, scale = 0.1, nu = 4)
        out[['k2.sd.image']] = LaplacesDemon::rhalft(1, scale = 0.1, nu = 4)
        ...
        out[['k1.animal']] = rep(mean(out$k1), N.Animal)
        out[['k2.animal']] = rep(mean(out$k2), N.Animal)
        ...
        out[['k1.sd.animal']] = LaplacesDemon::rhalft(1, scale = 0.1, nu = 4)
        out[['k2.sd.animal']] = LaplacesDemon::rhalft(1, scale = 0.1, nu = 4)
        return(out)
    }
    return(switch(chain,
                  "1" = gen_list(chain),
